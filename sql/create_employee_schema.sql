-- create_employee_schema.sql
-- Run in DBeaver connected to your MySQL server. Creates `employeeData` DB and required tables.
...
-- Notes:
-- - `ssn` is stored as 9-character string without dashes. Ensure client-side validation.
-- - `is_full_time` can be used to filter full-time employees in reports.
